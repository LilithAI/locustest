...
BEGIN
  ...
  IF v_stats.user_id IS NULL THEN
    v_stats_json := jsonb_build_object(
      'total_points',0,'accuracy_pct',0,'current_streak',0,
      'longest_streak',0,'total_attempts',0,'designation','trainee'
    );
  ELSE
    v_stats_json := jsonb_build_object(
  ...
  -- Use to_jsonb (row_to_jsonb does not exist in Postgres).
  SELECT COALESCE(jsonb_agg(to_jsonb(t) ORDER BY t.attempted_at DESC), '[]'::jsonb)
    INTO v_recent
  FROM (
    SELECT a.id, a.is_correct, a.points_awarded, a.attempted_at,
           c.title AS challenge_title, c.question_type
  ...
  IF v_stats.user_id IS NOT NULL AND v_stats.total_points > 0 THEN
    ...
    FROM public.bar_user_stats WHERE total_points > v_stats.total_points;
  END IF;
